...
* **Accessibility:** zkLogin is one of several native Sui signature schemes thanks to [Sui cryptography agility](/concepts/transactions/transaction-auth). It integrates with other Sui primitives, such as sponsored transactions and multisig.
...
## OpenID providers[​](#openid-providers "Direct link to OpenID providers")
...
## How zkLogin works[​](#how-zklogin-works "Direct link to How zkLogin works")
...
## The complete zkLogin flow[​](#the-complete-zklogin-flow "Direct link to The complete zkLogin flow")

![zkLogin flow diagram](/assets/images/zklogin-flow-cd594bdf380b6a3c7c7935c1258328e8.png "zkLogin complete flow")

#### Step 0: zkLogin uses Groth16 for zkSNARK instantiation, which requires a common reference string (CRS) linked to the circuit.[​](#step-0-zklogin-uses-groth16-for-zksnark-instantiation-which-requires-a-common-reference-string-crs-linked-to-the-circuit "Direct link to Step 0: zkLogin uses Groth16 for zkSNARK instantiation, which requires a common reference string (CRS) linked to the circuit.")
...
#### Steps 1-3: The user logs in to an OpenID provider (OP) to obtain a JWT containing a nonce.[​](#steps-1-3-the-user-logs-in-to-an-openid-provider-op-to-obtain-a-jwt-containing-a-nonce "Direct link to Steps 1-3: The user logs in to an OpenID provider (OP) to obtain a JWT containing a nonce.")
...
#### Steps 4-5: The application frontend sends the JWT to a salt service. The service returns the unique `user_salt` based on `iss`, `aud`, and `sub`.[​](#steps-4-5-the-application-frontend-sends-the-jwt-to-a-salt-service-the-service-returns-the-unique-user_salt-based-on-iss-aud-and-sub "Direct link to steps-4-5-the-application-frontend-sends-the-jwt-to-a-salt-service-the-service-returns-the-unique-user_salt-based-on-iss-aud-and-sub")

#### Steps 6-7: The user sends the JWT, user salt, ephemeral public key, JWT randomness, and key claim name (for example, `sub`) to the proving service.[​](#steps-6-7-the-user-sends-the-jwt-user-salt-ephemeral-public-key-jwt-randomness-and-key-claim-name-for-example-sub-to-the-proving-service "Direct link to steps-6-7-the-user-sends-the-jwt-user-salt-ephemeral-public-key-jwt-randomness-and-key-claim-name-for-example-sub-to-the-proving-service")
...
#### Step 8: The application computes the user address based on `iss`, `aud`, and `sub`.[​](#step-8-the-application-computes-the-user-address-based-on-iss-aud-and-sub "Direct link to step-8-the-application-computes-the-user-address-based-on-iss-aud-and-sub")

#### Steps 9-10: The user signs the transaction with the ephemeral private key and submits it with the ephemeral signature, ZK proof, and other inputs to Sui.[​](#steps-9-10-the-user-signs-the-transaction-with-the-ephemeral-private-key-and-submits-it-with-the-ephemeral-signature-zk-proof-and-other-inputs-to-sui "Direct link to Steps 9-10: The user signs the transaction with the ephemeral private key and submits it with the ephemeral signature, ZK proof, and other inputs to Sui.")
...
## Address definition[​](#address-definition "Direct link to Address definition")

The address is computed on the following inputs:

1. The address flag: `zk_login_flag = 0x05` for zkLogin address. This serves as a domain separator as a signature scheme defined in [crypto agility](/concepts/transactions/transaction-auth).
2. `kc_name_F = hashBytesToField(kc_name, maxKCNameLen)`: Name of the key claim, ex. `sub`. The sequence of bytes is mapped to a field element in BN254 using `hashBytesToField` (defined below).
3. `kc_value_F = hashBytesToField(kc_value, maxKCValueLen)`: The value of the key claim mapped using `hashBytesToField`.
4. `aud_F = hashBytesToField(aud, maxAudValueLen)`: The relying party (RP) identifier. See [definition](#terminology-and-notations).
5. `iss`: The OpenID Provider (OP) identifier. See [definition](#terminology-and-notations).
...
## Terminology and notations[​](#terminology-and-notations "Direct link to Terminology and notations")
...
### OpenID provider (OP)[​](#openid-provider-op "Direct link to OpenID provider (OP)")
...
### Relying party (RP) or client[​](#relying-party-rp-or-client "Direct link to Relying party (RP) or client")
...
### Subject identifier (sub)[​](#subject-identifier-sub "Direct link to Subject identifier (sub)")
...
### JSON Web Key (JWK)[​](#json-web-key-jwk "Direct link to JSON Web Key (JWK)")
...
### JSON Web Token (JWT)[​](#json-web-token-jwt "Direct link to JSON Web Token (JWT)")
...
### Key claim[​](#key-claim "Direct link to Key claim")
...
### Notations[​](#notations "Direct link to Notations")
...
3. `ext_eph_pk`: The byte representation of an ephemeral public key (`flag || eph_pk`). Size varies depending on the choice of the signature scheme (denoted by the flag, defined in [Signatures](/concepts/transactions/transaction-auth)).
...
## Ceremony[​](#ceremony "Direct link to Ceremony")
...
### What is the ceremony?[​](#what-is-the-ceremony "Direct link to What is the ceremony?")
...
### How was the ceremony performed?[​](#how-was-the-ceremony-performed "Direct link to How was the ceremony performed?")
...
## Finalization[​](#finalization "Direct link to Finalization")
...
### User salt[​](#user-salt "Direct link to User salt")
...
### Ephemeral private key[​](#ephemeral-private-key "Direct link to Ephemeral private key")
...
### What providers is zkLogin compatible with?[​](#what-providers-is-zklogin-compatible-with "Direct link to What providers is zkLogin compatible with?")
...
### How is a zkLogin Wallet different from a traditional private key wallet?[​](#how-is-a-zklogin-wallet-different-from-a-traditional-private-key-wallet "Direct link to How is a zkLogin Wallet different from a traditional private key wallet?")
...
### How is zkLogin different from MPC or Multisig wallets?[​](#how-is-zklogin-different-from-mpc-or-multisig-wallets "Direct link to How is zkLogin different from MPC or Multisig wallets?")
...
### If my OAuth account is compromised, what happens to my zkLogin address?[​](#if-my-oauth-account-is-compromised-what-happens-to-my-zklogin-address "Direct link to If my OAuth account is compromised, what happens to my zkLogin address?")
...
### If I lose access to my OAuth account, do I lose access to my zkLogin address?[​](#if-i-lose-access-to-my-oauth-account-do-i-lose-access-to-my-zklogin-address "Direct link to If I lose access to my OAuth account, do I lose access to my zkLogin address?")

Yes. You must be able to log into your OAuth account and produce a current JWT in order to use zkLogin.

### Does losing my OAuth credential mean the loss of funds in the zkLogin wallet?[​](#does-losing-my-oauth-credential-mean-the-loss-of-funds-in-the-zklogin-wallet "Direct link to Does losing my OAuth credential mean the loss of funds in the zkLogin wallet?")
...
### Can I convert or merge a traditional private key wallet into a zkLogin one, or vice versa?[​](#can-i-convert-or-merge-a-traditional-private-key-wallet-into-a-zklogin-one-or-vice-versa "Direct link to Can I convert or merge a traditional private key wallet into a zkLogin one, or vice versa?")

No. The zkLogin wallet address is derived differently compared to a private key address.

### Will my zkLogin address ever change?[​](#will-my-zklogin-address-ever-change "Direct link to Will my zkLogin address ever change?")
...
### Can I have multiple addresses with the same OAuth provider?[​](#can-i-have-multiple-addresses-with-the-same-oauth-provider "Direct link to Can I have multiple addresses with the same OAuth provider?")
...
### Is a zkLogin Wallet custodial?[​](#is-a-zklogin-wallet-custodial "Direct link to Is a zkLogin Wallet custodial?")
...
### Generating a ZK proof is expensive, is a new proof required to be generated for every transaction?[​](#generating-a-zk-proof-is-expensive-is-a-new-proof-required-to-be-generated-for-every-transaction "Direct link to Generating a ZK proof is expensive, is a new proof required to be generated for every transaction?")
...
### Does zkLogin work on mobile?[​](#does-zklogin-work-on-mobile "Direct link to Does zkLogin work on mobile?")
...
### Is account recovery possible if the user loses the OAuth credentials?[​](#is-account-recovery-possible-if-the-user-loses-the-oauth-credentials "Direct link to Is account recovery possible if the user loses the OAuth credentials?")
...
### What are some assumptions for the zkLogin circuit?[​](#what-are-some-assumptions-for-the-zklogin-circuit "Direct link to What are some assumptions for the zkLogin circuit?")
...
### How is zkLogin different from other solutions that support social login?[​](#how-is-zklogin-different-from-other-solutions-that-support-social-login "Direct link to How is zkLogin different from other solutions that support social login?")
...
### How to verify a zkLogin signature off-chain?[​](#how-to-verify-a-zklogin-signature-off-chain "Direct link to How to verify a zkLogin signature off-chain?")
...
### Can I use zkLogin inside a multisig wallet?[​](#can-i-use-zklogin-inside-a-multisig-wallet "Direct link to Can I use zkLogin inside a multisig wallet?")
...
[zkLogin Integration Guide](/guides/developer/cryptography/zklogin-integration)

zkLogin can be integrated into applications deployed on Sui.

•

[zkLogin Example](/guides/developer/cryptography/zklogin-integration/zklogin-example)
...
* [The complete zkLogin flow](#the-complete-zklogin-flow)
* [Entities](#entities)
* [Address definition](#address-definition)
* [Terminology and notations](#terminology-and-notations)
  + [OpenID provider (OP)](#openid-provider-op)
  + [Relying party (RP) or client](#relying-party-rp-or-client)
  + [Subject identifier (sub)](#subject-identifier-sub)
  + [JSON Web Key (JWK)](#json-web-key-jwk)
  + [JSON Web Token (JWT)](#json-web-token-jwt)
  + [Key claim](#key-claim)
  + [Notations](#notations)
* [Ceremony](#ceremony)
  + [What is the ceremony?](#what-is-the-ceremony)
  + [How was the ceremony performed?](#how-was-the-ceremony-performed)
* [Finalization](#finalization)
* [Security and privacy](#security-and-privacy)
  + [JWT](#jwt)
  + [User salt](#user-salt)
  + [Ephemeral private key](#ephemeral-private-key)
  + [Proof](#proof)
  + [Privacy](#privacy)
* [FAQ](#faq)
  + [What providers is zkLogin compatible with?](#what-providers-is-zklogin-compatible-with)
  + [How is a zkLogin Wallet different from a traditional private key wallet?](#how-is-a-zklogin-wallet-different-from-a-traditional-private-key-wallet)
  + [How is zkLogin different from MPC or Multisig wallets?](#how-is-zklogin-different-from-mpc-or-multisig-wallets)
  + [If my OAuth account is compromised, what happens to my zkLogin address?](#if-my-oauth-account-is-compromised-what-happens-to-my-zklogin-address)
  + [If I lose access to my OAuth account, do I lose access to my zkLogin address?](#if-i-lose-access-to-my-oauth-account-do-i-lose-access-to-my-zklogin-address)
  + [Does losing my OAuth credential mean the loss of funds in the zkLogin wallet?](#does-losing-my-oauth-credential-mean-the-loss-of-funds-in-the-zklogin-wallet)
  + [Can I convert or merge a traditional private key wallet into a zkLogin one, or vice versa?](#can-i-convert-or-merge-a-traditional-private-key-wallet-into-a-zklogin-one-or-vice-versa)
  + [Will my zkLogin address ever change?](#will-my-zklogin-address-ever-change)
  + [Can I have multiple addresses with the same OAuth provider?](#can-i-have-multiple-addresses-with-the-same-oauth-provider)
  + [Is a zkLogin Wallet custodial?](#is-a-zklogin-wallet-custodial)
  + [Generating a ZK proof is expensive, is a new proof required to be generated for every transaction?](#generating-a-zk-proof-is-expensive-is-a-new-proof-required-to-be-generated-for-every-transaction)
  + [Does zkLogin work on mobile?](#does-zklogin-work-on-mobile)
  + [Is account recovery possible if the user loses the OAuth credentials?](#is-account-recovery-possible-if-the-user-loses-the-oauth-credentials)
  + [What are some assumptions for the zkLogin circuit?](#what-are-some-assumptions-for-the-zklogin-circuit)
  + [How is zkLogin different from other solutions that support social login?](#how-is-zklogin-different-from-other-solutions-that-support-social-login)
  + [How to verify a zkLogin signature off-chain?](#how-to-verify-a-zklogin-signature-off-chain)
  + [Can I use zkLogin inside a multisig wallet?](#can-i-use-zklogin-inside-a-multisig-wallet)
