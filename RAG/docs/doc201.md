...
## Packages are immutable[​](#packages-are-immutable "Direct link to Packages are immutable")
...
## Upgrading packages[​](#upgrading-packages "Direct link to Upgrading packages")
...
See [Upgrading packages](/concepts/sui-move-concepts/packages/upgrade) for details on the process.

## Using the Sui Client CLI[​](#using-the-sui-client-cli "Direct link to Using the Sui Client CLI")

The [Sui Client CLI](/references/cli/client) provides an approachable way to upgrade packages. If the CLI's active address owns the `UpgradeCap` for a package, you can use the `upgrade` command to perform an upgrade.
...
### Making packages immutable[​](#making-packages-immutable "Direct link to Making packages immutable")
...
For a more flexible approach, use [custom upgrade policies](/concepts/sui-move-concepts/packages/custom-policies) to protect `UpgradeCap` access without giving up the ability to upgrade.

* [Packages are immutable](#packages-are-immutable)
* [Upgrading packages](#upgrading-packages)
* [Using the Sui Client CLI](#using-the-sui-client-cli)
  + [Making packages immutable](#making-packages-immutable)