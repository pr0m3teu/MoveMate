...
## Native token: SUI[​](#native-token-sui "Direct link to Native token: SUI")
...
## Stakeholders[​](#stakeholders "Direct link to Stakeholders")
...
### Delegated proof of stake[​](#delegated-proof-of-stake "Direct link to Delegated proof of stake")

Sui uses a delegated proof of stake (DPoS) [consensus mechanism](/concepts/sui-architecture/consensus) where [validators](/guides/operator/validator-index) lock a certain amount of SUI as collateral for the duration of an [epoch](/concepts/sui-architecture/epochs). Validators cannot make changes to their stake during an epoch, and any changes are applied when a new epoch begins. They then earn rewards for processing operations, such as validating transactions, and for providing resources.

Users of the network hold their own SUI, which they can delegate to validators of their choice as part of a validator's stake. In so doing, the validators reward users based on the amount of SUI they delegate. Users are free to withdraw their SUI or place stake in a different validator before a new [epoch](/concepts/sui-architecture/epochs) begins.

![Sui tokenomics flow](/assets/images/sui-tokenomics-flow-f63d253d408a181505f05465ddc39630.png "Flowchart showing the tokenomics structure.")

## Token supply[​](#token-supply "Direct link to Token supply")
...
## Token distribution[​](#token-distribution "Direct link to Token distribution")

At the beginning of each [epoch], three important events happen:

1. SUI holders stake their tokens to validators and a new [committee](/concepts/sui-architecture/consensus) is formed.

* The reference gas prices are set as described in [Sui Gas Fees](/concepts/tokenomics/gas-in-sui).
...
### Vesting schedules[​](#vesting-schedules "Direct link to Vesting schedules")
...
### Airdrops[​](#airdrops "Direct link to Airdrops")
...
## Storage fund[​](#storage-fund "Direct link to Storage fund")
...
### Storage fund rewards[​](#storage-fund-rewards "Direct link to Storage fund rewards")
...
### Deflation[​](#deflation "Direct link to Deflation")
...
## Validator rewards[​](#validator-rewards "Direct link to Validator rewards")
...
### Staking pools[​](#staking-pools "Direct link to Staking pools")
...
### Validator pool exchange rate[​](#validator-pool-exchange-rate "Direct link to Validator pool exchange rate")

The exchange rate for each validator pool is calculated at each epoch boundary as follows:

ExchangerateatE+1=(1+(third−partystakerrewardsatE/third−partystakeatE))×(exchangerateatE)Exchange rate at E+1 = (1 + (third-party staker rewards at E / third-party stake at E)) × (exchange rate at E)ExchangerateatE+1=(1+(third−partystakerrewardsatE/third−partystakeatE))×(exchangerateatE)
...
#### Understanding the voting power formula[​](#understanding-the-voting-power-formula "Direct link to Understanding the voting power formula")
...
[The Sui Smart Contracts Platform: Economics and Incentives](https://docs.sui.io/paper/tokenomics.pdf)
...
* [Native token: SUI](#native-token-sui)
...
* [Token distribution](#token-distribution)
  ...
  + [Airdrops](#airdrops)
* [Storage fund](#storage-fund)
  + [Storage fund rewards](#storage-fund-rewards)
  + [Deflation](#deflation)
* [Validator rewards](#validator-rewards)
  + [Staking pools](#staking-pools)
  + [Validator pool exchange rate](#validator-pool-exchange-rate)
